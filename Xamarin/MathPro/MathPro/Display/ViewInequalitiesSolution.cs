...
{
    public sealed class ViewInequalitiesSolution : ViewPlayerItem
    {
        #region Properties
        ...
        public ViewInequalitiesSolution(Inequalities inequalitySolution, Context context)
            :base(context)
        {
            _context = context;
            foreach (var t in inequalitySolution.Solution)
            {
	            if (t.GetStepType() == SolveForXStepType.FactorEquations)
	            {
	            ...
	            {
		            AddView(new Step((Equation) t, _context));
					AddView(SpaceWaster());
	            }
            }
        }
        ...
        {
            var pieces = new List<iEquationPiece>();
        ...
        public Sign Flip(Sign old)
        {
            switch (old.SignType)
            {
                ...
                case SignType.greaterEqual:
                    return new Sign(SignType.smallerEqual);
                case SignType.smaller:
                    return new Sign(SignType.greater);
                case SignType.smallerEqual:
