...
using MathBase;
using Type = MathBase.Type;

namespace MathPro.Display
{
    public sealed class ViewLogarithmicSolution : ViewPlayerItem
    {
        #region Properties
        ...
        public ViewLogarithmicSolution(SolveForLogarithmicEquations sol,Context context) 
            : base(context)
        {
            _context = context;
            foreach (var t in sol.Solution)
            {
                switch (t.GetStepType())
                {
                    case SolveForXStepType.Equation:
                        AddView(new Step((Equation)t,_context));//AddView(SpaceWaster());
                        break;
					case SolveForXStepType.LogEquation:
                        AddView(new Step((LogarithmicEquation)t,_context));//AddView(SpaceWaster());
                        break;
